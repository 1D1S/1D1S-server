...
public class ChallengeMutationService {
    ...
    public ChallengeDto acceptApplicants(Long challengeId, List<Long> applicantIds) {
        for (Long memberId : applicantIds) {
            MemberChallenge mc = memberChallengeRepository.findByMemberIdAndChallengeId(memberId, challengeId)
                    .orElseThrow(() -> new CustomException(ErrorCode.MEMBER_NOT_FOUND));
    ...
    public ChallengeDto rejectApplicants(Long challengeId, List<Long> applicantIds) {
        ...
        return ChallengeDto.from(challengeRepository.findById(challengeId).orElseThrow(() -> new CustomException(ErrorCode.CHALLENGE_NOT_FOUND)));
    }
    ...
    public int cancelChallengeLike(Long challengeId) {
        ChallengeLike like = challengeLikeRepository.findByChallengeIdAndMemberId(challengeId, currentMemberId);
        if (like != null) {
