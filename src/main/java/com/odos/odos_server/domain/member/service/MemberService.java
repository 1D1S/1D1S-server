...
import com.odos.odos_server.domain.diary.repository.DiaryGoalRepository;
...
public class MemberService {
  ...
  public StreakDto getStreakByMemberId(Long id) {
    ...
    return new StreakDto(todayGoalCount,currentStreak,totalDiaryCount,totalGoalCount,currentMonthDiaryCount, currentMonthGoalCount, maxStreak, calendar);
  }
  private List<DailyStreakDto> generateCalendar(List<Diary> diaryList) {
    YearMonth currentMonth = YearMonth.now();

    // 날짜별 개수를 저장할 Map
    Map<LocalDate, Long> diaryCountByDate = diaryList.stream()
  ...
  public int getTodayGoalCount(Long memberId) {
    ...
    return (int) allGoals.stream()
            .filter(goal -> {
              MemberChallenge mc = goal.getMemberChallenge();
              if (mc == null || mc.getMember() == null || mc.getChallenge() == null) return false;
              ...
              boolean notEnded = (end == null) || !today.isAfter(end); // today ≤ endDate or no endDate
  ...
  private int[] calculateStreaks(List<Diary> diaryList) {
    ...
    Set<LocalDate> diaryDates = diaryList.stream()
            .map(Diary::getDate)
    ...
    List<LocalDate> sortedDates = diaryDates.stream()
            .sorted()
            .toList();
